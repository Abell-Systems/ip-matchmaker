...
| **Known bug, NOT resolved**: final `candidates`/`verdicts`/`scorecards` often come back empty on `/api/analyze` | Reproduced **5/5 times** on 2026-08-30 against production. Root causes found and two fixes shipped, neither fully closes it: (1) `adversarial_agent` has no `output_schema` (can't combine with its `tools=[...]`), so Gemini sometimes returns markdown prose instead of JSON — fixed with an explicit JSON-only instruction in the prompt + a best-effort `_extract_json_object()` fallback (commit `b56c37b`). (2) A mid-run checkpoint can see real data (e.g. `scorecardsCount: 1`) that's gone by the final `get_session()` call — worked around by tracking the last *validated* snapshot of each state key during the run instead of trusting a final fetch (commits `b56c37b`, `980522f`). **Despite both fixes, the 5th real run still came back with 0 candidates** even though 4 `candidate_generated` events fired and `_validated()` logged zero warnings — meaning `_validated(InventionCandidate, cands)` itself returned empty without raising, for reasons not yet understood. Added one more diagnostic log line (commit `fd15db3`, pre-return snapshot dump) but did not verify it against a real run — paused here per the 2026-09-02 02:00 submission deadline. Demo script was reworked to rely on the adversarial rejection evidence trail (real cited prior art per rejection, which DID work reliably in all 5 runs) rather than a final ScoreCard, and that decision stands regardless of whether this bug gets fixed later. | — | Investigate post-hackathon; next step is reading the `fd15db3` log line from a real run |
