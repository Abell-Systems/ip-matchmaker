...
- **Definition of done:** a researcher can pick a cluster, see candidate inventions, see their scores, and expand "why" to see the cited patents behind both the adversarial verdict and the governor score. **Wiring complete:** `POST /api/analyze` runs the full agent graph for one cluster and returns candidates + verdicts + scorecards (rate-limited via `RateLimiter`, non-blocking), and `OpportunityMap` renders them — search form, expandable cluster cards, candidate/scorecard drill-down with citations. Remaining: one quota-budgeted live browser click-through (`INVENTION_LOOP_MAX_ITERATIONS=1` combo from Resolved decisions) to confirm the rendered drill-down against real agent output, then Cloud Run deploy (still credential-blocked).
