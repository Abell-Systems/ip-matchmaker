...
| **Known bug**: final `ScoreCard`/verdicts often come back empty on `/api/analyze` | Reproduced 3/3 times on 2026-08-30 against production. Two distinct causes found: (1) `adversarial_agent` has no `output_schema` (can't combine with its `tools=[...]`), so Gemini sometimes returns markdown prose instead of JSON for a verdict, which `_validated()` then drops (now logged, see `main.py::_validated`, commit `cf2eb97`). (2) Separately, one run's mid-execution events showed `scorecardsCount: 1` (governor did score something) but the final assembled result still had 0 scorecards, with no validation warning logged — meaning the value existed in `state` at one streaming checkpoint but was gone by the final `get_session()` call. Root cause of (2) not yet found; suspect an `InMemorySessionService` timing/persistence quirk with ADK's streamed events, not a schema problem. Decided not to chase this further before the hackathon deadline — the demo script was reworked to show the adversarial rejection evidence trail (real cited prior art per rejection) instead of depending on a final surviving ScoreCard. | — | Investigate post-hackathon |
